...
if($isDeployed)
{
	...
	try
	{
		...
		{
			$vm1.fqdn =  RunLinuxCmd -username $user -password $password -ip $hs1VIP -port $hs1vm1sshport -command "cat /etc/resolv.conf | grep search | awk '{print $2}'"
			$vm2.fqdn =  RunLinuxCmd -username $user -password $password -ip $hs2VIP -port $hs2vm1sshport -command "cat /etc/resolv.conf | grep search | awk '{print $2}'"
		}
