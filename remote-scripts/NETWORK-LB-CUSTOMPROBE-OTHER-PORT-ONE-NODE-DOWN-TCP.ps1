...
#Start Iperf Server on Custom Probe 
			...
			$isServer1Started = IsIperfServerStarted -node $server1  $expectedServerInstances
			$isServer2Started = IsIperfServerStarted -node $server2  $expectedServerInstances
...
#Step4 : Wait for 45 sec and then Start Iperf Client Again
					sleep($wait)
...
#Start iperf Server on Custom Probe Again, as starting Iperfserver in above command have stopped running iperf server
					...
					$server1.cmd = $cmd11
					StartIperfServer -node $server1
#Step7 : Wait for 45 sec and then Start Iperf Client Again
					sleep($wait)
