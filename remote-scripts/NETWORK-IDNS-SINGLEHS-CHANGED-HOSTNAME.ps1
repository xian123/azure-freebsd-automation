...
if($isDeployed)
{
	...
	foreach ($mode in $currentTestData.TestMode.Split(","))
	{
		...
		try
		{
            ...
            {
				$vm1DefaultFqdn = RunLinuxCmd -username $user -password $password -ip $hs1VIP -port $hs1vm1sshport -command "cat /etc/resolv.conf | grep search | awk '{print $2}'"
                $vm2DefaultFqdn = RunLinuxCmd -username $user -password $password -ip $hs1VIP -port $hs1vm2sshport -command "cat /etc/resolv.conf | grep search | awk '{print $2}'"
