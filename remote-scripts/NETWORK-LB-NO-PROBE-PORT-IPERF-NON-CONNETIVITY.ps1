...
#>>>On confirmation, of server starting, let's start iperf client...
				...
				{
					LogErr "Ohh, client got connected .. Thats wierd.. Looking if it is connected to server or not?"
					...
					if(($server1State -eq $true) -and ($server2State -eq $true)) {
						LogErr ".. whoa, whoa, whoa.!!. client is connected to sever. THATS NOT EXPECTED."
