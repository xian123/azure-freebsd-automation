...
if($isDeployed)
{
	...
	$pSize = 6
	$cmd1="python start-server.py -p $testPort && mv -f Runtime.log start-server.py.log"
	$cmd2="python start-server.py -p $testPort && mv -f Runtime.log start-server.py.log"
	...
	{
		LogMsg "Test Started for $mode mode.."
		try
		{
			$testResult = $null
...
#LogMsg "Failured detected in client connection."
					LogErr "Client connected.. [not expected]. Verifying that it connected to server.."
					$server1State = IsIperfServerRunning $server1
					$server2State = IsIperfServerRunning $server2
